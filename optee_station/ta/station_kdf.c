...
static const char MASTER_KEY_OBJ_ID[] = "station_master_key";
static const char SEQ_OBJ_ID[]        = "station_seq_counter";

static uint8_t  g_master_key[MASTER_KEY_SIZE]= {
    0x9f, 0x4e, 0x38, 0xf0, 0x52, 0xcd, 0x51, 0x87,
    0xe8, 0xbf, 0x69, 0xcd, 0x31, 0x73, 0xd2, 0xa9,
    0x12, 0xd9, 0x22, 0x73, 0xa8, 0x39, 0x13, 0x2f,
    0xa8, 0xf9, 0x44, 0x6d, 0xa6, 0x51, 0x3b, 0xea,
};
...
static TEE_Result load_or_create_master_key(void)
{
	...
	//IMSG("master key size:%d\n", g_master_key);
	//uint32_t key_val = 1073981072;
	//memcpy(g_master_key, &key_val, sizeof(key_val));
                                  ...
                                  uint8_t out_key[DERIVED_KEY_SIZE])
{
	...
	static const uint8_t salt[] = "STATION-HKDF-SALT";
	static const uint8_t info_prefix[] = "STATION-LOG-V1";
...
TEE_Result station_kdf_init(void)
{
	TEE_Result res;
...
TEE_Result station_kdf_next_key(uint64_t *seq_out,
                             uint8_t out_key[DERIVED_KEY_SIZE])
{
	TEE_Result res;
