...
TEE_Result TA_CreateEntryPoint(void)
{
	...
	res = nmea_kdf_init();
	if (res != TEE_SUCCESS) {
		EMSG("nmea_kdf_init failed: 0x%x", res);
		return res;
	}
...
static TEE_Result process_line(uint32_t param_types, TEE_Param params[4])
{
	...
	/* 1) 派生這筆資料的 key */
	res = nmea_kdf_next_key(&seq, derived_key);
	if (res != TEE_SUCCESS)
		return res;
	...
	/* 4) 輸出 record 空間檢查 */
	/* version(1) + seq(8) + nonce(12) + ct_len(4) + ciphertext + tag(16) */
	...
	u32_to_be(ciphertext_len, ct_len_be);
